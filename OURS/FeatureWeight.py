...
def Saliency_2(img):
    ...
    sm = np.zeros(l.shape, l[0][1].dtype)
    # -------------------------------
    l = cv2.subtract(l, lm) / 255.0
    a = cv2.subtract(a, am) / 255.0
    b = cv2.subtract(b, bm) / 255.0
...
def LaplacianContrast_2(img):
    # img=cv2.CreateMat(h, w, cv2.CV_32FC3)
    laplacian = cv2.Laplacian(img,5)
    laplacian = cv2.convertScaleAbs(laplacian)
    t_mean = np.mean(laplacian)
    if t_mean < 1e-6:
        t_mean = 1e-5
    laplacian_2 = laplacian / ( 6 * t_mean)
    return laplacian_2
...
def LocalContrast_2(img):

    lab_img = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
    L = cv2.extractChannel(lab_img, 0)
    L = np.float32(L)
    h = [1.0 / 16.0, 4.0 / 16.0, 6.0 / 16.0, 4.0 / 16.0, 1.0 / 16.0]
    mask = np.ones((len(h), len(h)), L[0][0].dtype)
    for i in range(len(h)):
        for j in range(len(h)):
            mask[i][j] = (h[i] * h[j])
    localContrast = cv2.filter2D(L, 5, kernel=mask)
    '''
    for i in range(len(localContrast)):
        for j in range(len(localContrast[0])):
            if localContrast[i][j] > math.pi / 2.75:
    ...
    localContrast = cv2.subtract(L, localContrast) / 8.0

    for i in range(len(localContrast)):
        for j in range(len(localContrast[0])):
            if localContrast[i][j] < 0:
                localContrast[i][j] = 0.0
